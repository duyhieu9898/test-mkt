...
import { AlertCircle, ArrowLeft, ExternalLink, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export default function FacebookAdDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const result = await api.get<{ data: Detail }>(`/ads/company/${companyId}/facebook/campaigns/${campaignId}/ads/${adId}`, { token });
      setDetail(result.data);
    } catch (error) {
      const msg = (error as Error).message || 'Could not load this ad.';
      setLoadError(msg);
      toast.error(msg);
    }
  }, [adId, campaignId, companyId, token]);
  ...
  if (loadError)
    return (
      <div className="mx-auto max-w-5xl space-y-4 py-8 text-center">
        ...
        <h2 className="text-lg font-bold">Could not load ad</h2>
