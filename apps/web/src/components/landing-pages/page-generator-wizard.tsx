...
export function PageGeneratorWizard({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, role: string, idx?: number) => {
    ...
    try {
      const data = await uploadImageAsset(companyId, file, token);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      {/* Progress Steps */}
      <div className="grid shrink-0 grid-cols-6 gap-1 border-b px-4 py-3 sm:px-6">
        {STEPS.map((s) => {
          ...
          return (
            <button
              ...
              className={`flex min-w-0 items-center justify-center gap-1 rounded-md px-1 py-2 text-xs font-medium transition-colors ${
                ...
                >
                  {attachedDoc ? (
                    <div className="flex min-w-0 items-center justify-center gap-2">
                      ...
                      <span className="min-w-0 truncate text-sm font-medium">{attachedDoc.name}</span>
