...
export const landingPages = pgTable(
  'landing_pages',
  {
    ...
    // Built-in subdomain hosting (doc 10 §L3 — Đợt 5)
    // `subdomain` is the unique path segment at /pages/:subdomain.
    // Changes flow through an admin approval queue — users never push
    // live without review. Future: wildcard DNS can rewrite to subdomain.
    subdomain: varchar('subdomain', { length: 100 }).unique(),
    publishApprovalStatus: varchar('publish_approval_status', { length: 20 }).default('draft'),
    publishSubmittedAt: timestamp('publish_submitted_at'),
    publishApprovedAt: timestamp('publish_approved_at'),
    publishApprovedBy: uuid('publish_approved_by'),
  ...
  (table) => ({
    ...
    subdomainIdx: index('landing_pages_subdomain_idx').on(table.subdomain),
    publishStatusIdx: index('landing_pages_publish_status_idx').on(table.publishApprovalStatus),
  })
);

// =============================================================================
// LANDING PAGE BLOG POSTS (doc 10 §L4 — Đợt 5)
// Each landing page can host its own blog at /pages/:subdomain/blog/:slug.
// Content is stored as HTML generated by TipTap on the client.
// =============================================================================

export const landingPageBlogPosts = pgTable(
  'landing_page_blog_posts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
